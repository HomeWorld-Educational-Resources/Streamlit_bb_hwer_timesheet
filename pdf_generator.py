...
def _contractor_group_table(group):
    """Build a Customer | Hours | Amount table for one contractor, with a subtotal row.
    group = {contractor, rate, rows: [{customer, hours, amount}], total_hours, total_amount}"""
    rate = group.get('rate')
    col_w = [CW - 90 - 90, 90, 90]
    header = [
        Paragraph('<b>Customer</b>', BOLD8GOLD),
        Paragraph('<b>Hours</b>', BOLD8GOLD),
        Paragraph('<b>Amount</b>', BOLD8GOLD),
    ]
    rows = [header]
    for r in group['rows']:
        rows.append([
            Paragraph(r['customer'] or 'Unassigned', BODY8),
            f"{r['hours']:g}",
            f"${r['amount']:,.2f}" if rate else '—',
        ])
    rows.append([
        Paragraph('<b>Subtotal</b>', BOLD8),
        Paragraph(f"<b>{group['total_hours']:g}</b>", BOLD8),
        Paragraph(f"<b>${group['total_amount']:,.2f}</b>" if rate else '<b>—</b>', BOLD8),
    ])
...
def generate_contractor_summary_pdf(period_label, grouped, grand_hours, grand_amount, logo_path=None):
    """
    period_label – e.g. 'Jan 1, 2026 – Jan 31, 2026'
    grouped       – list of {contractor, rate, rows: [{customer, hours, amount}], total_hours, total_amount}
    Returns bytes.
    """
    buf = io.BytesIO()
    c = pdf_canvas.Canvas(buf, pagesize=A4)
    c.setTitle('Contractor Payment Summary')
    c.setAuthor('BaoBunny')
    d = _Draw(c)
    ...
    c.setFillColor(white)
    c.drawString(ML + logo_offset, PAGE_H - 32, 'CONTRACTOR PAYMENT SUMMARY')
    c.setFont('Helvetica', 9)
    c.setFillColor(GOLD)
    c.drawString(ML + logo_offset, PAGE_H - 44, period_label)

    d.y = PAGE_H - 58 - 12

    # ── grand total summary boxes ─────────────────────────────────────────────
    _summary_boxes(d, [
        ('Contractors',   str(len(grouped))),
        ('Total Hours',   f'{grand_hours:g} hrs'),
        ('Total Amount Owed', f'${grand_amount:,.2f}'),
    ])
    d.rule(thickness=1.5, color=GOLD)

    # ── per-contractor tables ─────────────────────────────────────────────────
    for g in grouped:
        table = _contractor_group_table(g)
        _, th = table.wrapOn(c, CW, PAGE_H)
        rate = g.get('rate')
        rate_label = f"${rate:,.2f}/hr" if rate else 'rate not set'
        need = th + 30
        if d.y - need < MB + 20:
            d.new_page()
        d.section_label(f"{g['contractor']}  ·  {rate_label}", pad_above=16)
        d.place_table(table)
    ...
    c.setFont('Helvetica-Bold', 7)
    c.setFillColor(MID_GRAY)
    c.drawCentredString(PAGE_W / 2, fy - 14,
                        f'Generated by BaoBunny  ·  {datetime.now().strftime("%Y-%m-%d %H:%M")}')
